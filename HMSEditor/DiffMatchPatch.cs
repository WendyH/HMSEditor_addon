    ...
    */
    ...
    public class diff_match_patch
    {
                                        ...
                                        string text1, string text2)
        {
            ...
            List<Diff> list = new List<Diff>(path.Count);
            foreach (Diff instance in path)
                list.Add(instance);
            return list;
        }
                ...
                switch (aDiff.operation)
                {
                    ...
                    case Operation.EQUAL:
                        if (aDiff.text.Length <= 2 * Patch_Margin
                            && patch.diffs.Count != 0 && aDiff != diffs[diffs.Count - 1])
                        {
        ...
        public string patch_addPadding(List<Patch> patches)
        {
            ...
            diffs = patch.diffs;
            if (diffs.Count == 0 || diffs[diffs.Count - 1].operation != Operation.EQUAL)
            {
                // Add nullPadding equality.
